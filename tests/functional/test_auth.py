...
def test_login_action(test_client, init_database):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/auth/login' page is posted to with valid credentials
...
def test_login_action_already_logged_in(test_client, init_database, log_in_default_user):
    ...
    WHEN the '/auth/login' page is posted to
...
def test_invalid_login(test_client, init_database):
    ...
    WHEN the '/auth/login' page is posted to with invalid login details
    THEN 
    """
    response = test_client.post('/auth/login',
...
def test_missing_password_login(test_client_request):
    ...
    with test_client_request.test_request_context("/auth/login", method="POST", data={"username": "test_user", "password":""}):
...
def test_missing_username_login(test_client_request):
    ...
    with test_client_request.test_request_context("/auth/login", method="POST", data={"username": "", "password":"test123$"}):
