...
def test_home_page(test_client):
    ...
    WHEN the '/general/' page is requested (GET)
    ...
    """
    response = test_client.get('/general/home')
...
def test_confirm_page(test_client):
    ...
    WHEN the '/general/confirm' page is requested (GET)
    THEN the '200' status code is returned 
    """    

    response = test_client.get('/general/confirm', follow_redirects=True)
...
def test_contact(test_client):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/contact' page is requested (GET)
    THEN the '200' status code is returned 
    """    

    response = test_client.get('/general/contact')
    html = response.data.decode("utf-8")
...
def test_about(test_client):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/about' page is requested (GET)
    THEN the '200' status code is returned 
    """    

    response = test_client.get('/general/about')
    html = response.data.decode("utf-8")
...
def test_gallery(test_client):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/gallery' page is requested (GET)
    THEN the '200' status code is returned 
    """    

    response = test_client.get('/general/gallery')
    html = response.data.decode("utf-8")
...
def test_signup_error(test_client):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/error' page is requested (GET)
    THEN the '200' status code is returned 
    """    

    response = test_client.get('/general/error')
...
def test_signup_action_success(test_client, init_database):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/home' is successfully posted to with sign up data for the mailing list
    THEN the user is redirected to the 'general/confirm' page
    """
    data = {"name":'Polly Pocket', "email":'polly.pocket@gmails.com'}      
    response = test_client.post('/general/home', 
                                data=data, follow_redirects=True) 

    assert response.status_code == 200
    assert response.request.path == '/general/confirm'
...
def test_about_page_method_not_allowed(test_client):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/about' page is posted to (POST)
    THEN a '405' (Method Not Allowed) status code is returned 
    """        
    response = test_client.post('/general/about')
    assert response.status_code == 405

def test_gallery_page_method_not_allowed(test_client):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/gallery' page is posted to (POST)
    THEN a '405' (Method Not Allowed) status code is returned 
    """        
    response = test_client.post('/general/gallery')
    assert response.status_code == 405

def test_email_already_registered(test_client, init_database):
    """
    GIVEN a Flask application has been configured for testing
    WHEN the '/general/home' is posted to with sign up data that already exists in the mailing list
    ...
    assert response.status_code == 200
    assert response.request.path == '/general/home'
...
def test_register_missing_name(test_client_request, init_database):
    ...
    """ 
    with test_client_request.test_request_context("/general/home", method="POST", data={"name": "", "email":"lucy@gmails.com"}):
...
def test_register_missing_email(test_client_request, init_database):
    ...
    THEN the user is shown a validation error
    """ 
    with test_client_request.test_request_context("/general/home", method="POST", data={"name": "Lucy Doe", "email":""}):
...
def test_register_name_less_than_min_length(test_client_request, init_database):
    ...
    THEN the user is shown a validation error
    """              
    with test_client_request.test_request_context("/general/home", method="POST", data={"name": "", "email":"janey.doey@gmails.com"}):
...
def test_register_name_exceeds_max_length(test_client_request, init_database):
    ...
    THEN the user is shown a validation error
    """              
    with test_client_request.test_request_context("/general/home", method="POST", data={"name": long_name, "email":"janey.doey@gmails.com"}):
        request.form["name"] == ['Name must be between 1 and 100 characters.']
...
def test_register_email_less_than_min_length(test_client_request, init_database):
    ...
    THEN the user is shown a validation error
    """              
    with test_client_request.test_request_context("/general/home", method="POST", data={"name": "Janey Doe", "email":"j@"}):
...
def test_register_email_exceeds_max_length(test_client_request, init_database):
    ...
    THEN the user is shown a validation error
    """              
    with test_client_request.test_request_context("/general/home", method="POST", data={"name": "Janey Doe", "email": long_email}):
        request.form["name"] == ['Email must be between 3 and 255 characters.']
...
def test_register_improper_email(test_client_request, init_database, log_in_default_user):
    ...
    THEN the user is shown a validation error
    """
    with test_client_request.test_request_context("/general/home", method="POST", data={"name": "Pat Butcher", "email":"pat.butcher.gmails.com"}):
